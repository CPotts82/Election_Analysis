...
### Modification 2
2- This script could also be modified to keep up the with state or national election votes associated with political party ties. Using the example of the national presidential election again, we could keep up the number of voters per political party (ex: republican, democrat, green, libertarian, etc). We could use the data from the election data file and create a Series or list. The list would consist of the election candidates and an indentifier that shows what political party they are associated with.  As we configure and analysis the results of the candidates we could simultaneously be calculating the numbers and percentages of political party votes.  
